...
def start(start_date):
    ...
    results = session.query(*start_results).filter(measurements.date >= start_date).all()
...
def start_end(start_date,end_date):
    ...
    """Return min, max, avg for all tobs at start date and end date """
    # Query all precipitation
    start_end=[func.min(measurements.tobs),func.avg(measurements.tobs),func.max(measurements.tobs)]    
    start_end_results=session.query(*start_end).filter(measurements.date>=start_date).filter(measurements.date<=end_date).all()
